 ...
 * 
 * <h2>Purpose</h2>
 * <p>Creates two clickable buttons in the dashboard (AdvantageScope/Shuffleboard) that drivers
 * use to log whether each shot hit or missed the target. This provides the critical feedback
 * data that the Bayesian optimizer needs to improve shooting accuracy.</p>
 * 
 * <h2>Dashboard Button Locations</h2>
 * <ul>
 *   <li><b>/FiringSolver/LogHit</b> - Click when shot HITS the target (✅ GREEN button)</li>
 *   <li><b>/FiringSolver/LogMiss</b> - Click when shot MISSES the target (❌ RED button)</li>
 * </ul>
 * 
 * <h2>How It Works</h2>
 * <ol>
 *   <li>Driver observes shot result (hit or miss)</li>
 *   <li>Driver clicks appropriate button in dashboard</li>
 *   <li>This subsystem detects the button press in periodic()</li>
 *   <li>Calls {@link FiringSolutionSolver#logShotResult(boolean)} to log to AdvantageKit</li>
 *   <li>Sets ShotLogged flag for {@link ShooterInterlock} (if interlock enabled)</li>
 *   <li>Resets button to OFF automatically</li>
 * </ol>
 * 
 * <h2>Integration with Tuner</h2>
 * <p>The Python Bayesian tuner reads the logged hit/miss data from NetworkTables along with
 * all the robot state at shot time (distance, velocity, angles, current coefficients) to
 * learn patterns and optimize shooting parameters.</p>
 * 
 * <h2>Maintenance Notes</h2>
 * <ul>
 *   <li>This subsystem must be instantiated in RobotContainer for buttons to work</li>
 *   <li>Buttons automatically reset after each click (no driver cleanup needed)</li>
 *   <li>Edge detection (lastXValue) prevents multiple logs from single click</li>
 *   <li>ShotLogged flag integrates with optional shooting interlock system</li>
 * </ul>
 * 
 * <h2>Example Usage</h2>
 * <pre>{@code
...
public class ShotResultLogger extends SubsystemBase {
   ...
   * <ul>
   *   <li><b>/FiringSolver/LogHit</b> - Hit button (driver clicks, system reads)</li>
   *   <li><b>/FiringSolver/LogMiss</b> - Miss button (driver clicks, system reads)</li>
   *   <li><b>/FiringSolver/Interlock/ShotLogged</b> - Signals shot was logged (system writes)</li>
   * </ul>
   * 
   * <p>All buttons start in the OFF (false) state. The ShotLogged flag starts TRUE
   * so the first shot is allowed (if interlock is enabled).
   */
   ...
   * <ol>
   *   <li>Read current button state from NetworkTables</li>
   *   <li>Compare to previous state (lastXValue)</li>
   *   <li>If changed from false→true, driver just clicked!</li>
   *   <li>Process the click (log result, set flags, reset button)</li>
   *   <li>Update lastXValue for next cycle</li>
   * </ol>
   * 
